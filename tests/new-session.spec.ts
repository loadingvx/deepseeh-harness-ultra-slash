...
import {
  ...
  newSlashMatchSpace,
  startNewSession,
} from '../src/client/new-session.ts'
import type { SlashSource, SlashTriggerService } from '../src/slash-menu.ts'
import { translate } from '../src/locales.ts'

function zhT(key: string, vars?: Record<string, string | number>): string {
  return translate('zh', key as keyof typeof import('../src/locales.ts').zh, vars)
}
